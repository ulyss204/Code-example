...
export class MessengerDemo {
  ...
  //  Messenger().post('Thanks for checking out Messenger!');
    ...
    //jQuery('#show-error-message').on('click', function(): boolean {
      ...
      string = '' + string;
      i = 0;
      if(string.indexOf('401')){
        Messenger().post({
          message: 'The login / password do not match, please try again.  If you wish to change your password, please let us know at: support@tastemap.com',
          type: 'error',
      ...
      }else if(string.indexOf('500') || string.indexOf('503')){
        Messenger().post({
          message: "TasteMap's server is currently in maintenance and should be back shortly. If the problem persists, please reach us out at support@tastemap.com",
          type: 'error',
          showCloseButton: true
        });
      }
